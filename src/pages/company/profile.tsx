...
const CompanyProfilePage = () => {
  ...
  const handleGbizSearch = async () => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/company/gbiz-search', {
        method: 'POST',
  ...
  const handleSelectCompany = async (company: GbizSearchResult) => {
    ...
    setError(null);
    try {
      const response = await fetch('/api/company/gbiz-fetch-details', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto p-4 sm:p-8">
        <h1 className="text-3xl font-bold mb-8 text-center">{isNewProfile ? '企業プロフィール登録' : '企業プロフィール編集'}</h1>
