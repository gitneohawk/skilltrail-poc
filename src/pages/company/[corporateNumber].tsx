  ...
  useEffect(() => {
    if (corporateNumber) {
      const fetchCompanyProfile = async () => {
        ...
        setError(null);
        try {
          const response = await fetch(`/api/company/${corporateNumber}`);
      ...
      <div className="bg-white">
             ...
             <div className="bg-white p-2 rounded-full shadow-lg">
                {profile.logoUrl ? (
                    <img
                        ...
                        className="h-32 w-32 rounded-full object-contain border-2 border-white"
                        onError={(e) => { (e.target as HTMLImageElement).src = `https://placehold.co/128x128/e2e8f0/334155?text=Logo`; }}
                    />
                ) : (
                    <div className="h-32 w-32 rounded-full bg-slate-200 flex items-center justify-center text-slate-500">
                        <span>Logo</span>
                    </div>
                )}
             </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 divide-y divide-slate-200">
          ...
          <PageSection title="働く魅力・カルチャー" icon={SparklesIcon}>
            ...
            <p className="mb-6">{profile.mission || '未設定'}</p>
